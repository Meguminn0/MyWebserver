...
private:
    ...
    long long m_read_idx;               /* 读取的最后一个数据在readbuff中的位置 */
    char writeBuff[WRITEBUFF_MAX_LEN];
    long long m_write_idx;              /* 写入的最后一个数据在readbuff中的位置 */
    CHECK_STATE m_check_state;          /* 当前执行检查的动作 */
    long long m_check_idx;              /* 标注解析内容的位置 */
    long long m_line_idx;               /* 解析数据时，当解析行在buff中的位置 */
